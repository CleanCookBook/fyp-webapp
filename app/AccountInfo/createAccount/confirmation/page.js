...
const Confirmation = () => {
  ...
  const createUserAccount = async () => {
    if (isChecked) {
      try {
        const response = await fetch('https://cleancookbook.vercel.app/api/user/create-account', {
          method: 'POST',
          headers: {
  ...
  const createNutritionistAccount = async () => {
    ...
    try {
      const response = await fetch('https://cleancookbook.vercel.app/api/user/create-account-n', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },

    ...
    <div className="h-full bg-[#F9D548] text-[#0A2A67]">
      <Navbar userRole={userRole}/>
        <div className="flex flex-col bg-[#F9D548] text-[#0A2A67] justify-center items-center">
          <Link href="/AccountInfo/createAccount">
            <button className="text-2xl w-28 h-10 bg-blue-950 hover:bg-[#154083] text-white text-xl font-bold rounded-[10px] shadow self-start absolute top-28 left-10">
              &lt; Back
            </button>
          </Link>
        </div>
        <div className="lg:w-4/5 lg:pr-8 p-8 flex flex-col items-center">
          <h2 className="text-6xl font-black py-5 -mt-4 ml-80">
            Help us get to know you better!
          </h2>
        </div>
        
        <form action="/create-account" method="POST" onSubmit={handleSubmit}>
        <div className="lg:pr-8 p-8 grid grid-cols-3 gap-6 place-items-stretch -mt-12">
      ...
      <div className="flex justify-end items-end py-6">
          ...
          <button
            className="w-64 h-10 bg-blue-950 hover:bg-[#154083] text-white text-xl font-bold self-end rounded-[10px] shadow mr-8 -mt-32"

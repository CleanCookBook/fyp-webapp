...
const EditSysAccount = () => {
    ...
    const submitForm = async (e) => {
        ...
        try {
            const response = await fetch("http://localhost:3001/api/edit/update-profile", {
                method: "POST",
                headers: {
...
useEffect(() => {
    ...
    const fetchUserData = async () => {
        try {
          ...
          const response = await fetch(`http://localhost:3001/api/edit/${userID}`);
          const data = await response.json();
...
<div className="flex flex-col h-full bg-[#F9D548]">
    ...
    <form onSubmit={submitForm}>
        <div>
            <h2 className="text-xl underline font-bold left-10 mt-9">Edit Account Information:</h2>
            <div className="flex flex-row mt-9 gap-4">
                <label className="flex flex-row text-xl text-black font-semibold mt-2">Username :</label>
                <input 
            ...
            <div className="flex flex-row mt-9 gap-4">
                <label className="flex flex-row text-xl text-black font-semibold mt-2">First Name :</label>
                <input 
                  className="text-xl text-black w-[748px] h-12 rounded-[10px] border p-2" 
                  type="text" 
                  name="FName" 
            ...
            <div className="flex flex-row mt-9 gap-4">
                <label className="flex flex-row text-xl text-black font-semibold mt-2">Last Name :</label>
                <input 
                  className="text-xl text-black w-[748px] h-12 rounded-[10px] border p-2" 
                  type="text" 
                  name="LName" 
